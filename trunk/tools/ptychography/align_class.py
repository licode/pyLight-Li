...
[nr*usfac nc*usfac]
   ...
   b = npy.zeros([1,noc])
   b[0,:] = (npy.arange(noc)-coff)[:]
   kernc = npy.exp((-1j*2*npy.pi/(nc*usfac))*npy.dot(a,b))
   nndim = kernc.shape
   #print nndim

   a = npy.zeros([nor,1])
   a[:,0] = (npy.arange(nor)-roff)[:]
   b = npy.zeros([1,nr])
   b[0,:] = (sf.ifftshift(npy.arange(nr))-npy.floor(1.*nr/2))[:]
...
def subpixel_align(ref,tmp,threshold_flag,threshold, subpixel_flag):
    ...
    return sf.ifftshift(sf.fftn(sf.fftshift(tmp_fft_new)))

    
def remove_phase_ramp(tmp,threshold_flag, threshold,subpixel_flag):
   tmp_tmp = subpixel_align(sf.ifftshift(sf.ifftn(sf.fftshift(npy.abs(tmp)))), sf.ifftshift(sf.ifftn(sf.fftshift(tmp))), threshold_flag, threshold,subpixel_flag) 
