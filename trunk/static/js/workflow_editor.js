        ...
        $.each( data.data_outputs, function( i, output ) {
            ...
            if (node.type == 'tool'){
                var callout = $("<div class='callout "+label+"'></div>")
                    ...
                    .append(
                        $("<div class='buttons'></div>").append(
                            $("<img/>").attr('src', '/static/images/asterisk-small-outline.png').click( function() {
                                if ($.inArray(output.name, node.workflow_outputs) != -1){
                                    node.workflow_outputs.splice($.inArray(output.name, node.workflow_outputs), 1);
                                    callout.find('img').attr('src', '/static/images/asterisk-small-outline.png');
                                }else{
                                    node.workflow_outputs.push(output.name);
                                    callout.find('img').attr('src', '/static/images/asterisk-small.png');
                                }
                ...
                r.hover(
                    function(){
                        callout.find('img').attr('src', '/static/images/asterisk-small-yellow.png');
                    },
                    function(){
                        if ($.inArray(output.name, node.workflow_outputs) === -1){
                            callout.find('img').attr('src', '/static/images/asterisk-small-outline.png');
...
function prebuild_node( type, title_text, tool_id ) {
    var f = $("<div class='toolFormInCanvas'></div>");
    var node = new Node( f );
    node.type = type;
    if ( type == 'tool' ) {
    ...
    var tmp = "<div><img height='16' align='middle' src='/static/images/loading_small_white_bg.gif'/> loading tool info...</div>";
    b.append( tmp );
    node.form_html = tmp;
    f.append( b );
    ...
    buttons.append( $("<img/>").attr("src", '/static/images/delete.png').click( function( e ) {
        node.destroy();
    } ).hover( 
        function() { $(this).attr( "src", "/static/images/delete.png" ); },
        function() { $(this).attr( "src", "/static/images/delete.png" ); }
    ) );
